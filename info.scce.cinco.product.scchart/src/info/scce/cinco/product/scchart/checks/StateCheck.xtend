...
class StateCheck extends SCChartCheck{
	...
	override check(SCChart scchart) {
		...
		checkConnector(scchart)
	}
	
	def checkNameConvention(SCChart scchart){
		scchart.find(SimpleState).forEach[
	...
	def checkConnector(SCChart scchart){
		scchart.find(Connector).forEach[
			if(it.getOutgoing.size<1){
				it.addError("connector must have an outgoing transition")
			}
		]
	}
}