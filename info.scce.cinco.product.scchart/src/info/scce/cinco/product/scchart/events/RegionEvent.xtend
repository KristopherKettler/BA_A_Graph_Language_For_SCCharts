			...
			if (it.uuid == element.uuid) {
				...
				if (element.rootElement.rootStates.head.regions.size == 1) {
					...
					for (region1 : element.rootElement.rootStates.head.regions) {
						...
						if ((it.x - region1.x - region1.width <= 13 && it.x - region1.x - region1.width > -1) &&
							continue && region1.y < it.y && region1.y + region1.height > it.y &&
							it.uuid != region1.uuid) {
							var fix = 0
							if(region1.width % 2 == 1) fix = 1
							region1.width = (region1.width - 10) / 2
							it.width = region1.width + fix
							it.height = region1.height
							it.x = region1.x + region1.width + 10
							it.y = region1.y
							continue = false
						} // new region near right rim with half width 
						else if ((it.x <= 10) && continue && region1.y < it.y && region1.y + region1.height > it.y &&
							it.uuid != region1.uuid) {
							var fix = 0
							if(region1.width % 2 == 1) fix = 1
							region1.width = (region1.width - 10) / 2
						...
						else if ((it.y - region1.y - region1.height <= 13 && it.y - region1.y - region1.height > -1) &&
							continue && region1.x < it.x && region1.x + region1.width > it.x &&
							it.uuid != region1.uuid) {
							var fix = 0
							if(region1.height % 2 == 1) fix = 1
							region1.height = (region1.height - 10) / 2
							it.width = region1.width
							it.height = region1.height + fix
							it.x = region1.x
							it.y = region1.y + region1.height + 10
							continue = false
						} // new region near right rim with half height ref. region
						else if ((it.y < 33 + declarationCount * 13) && continue && region1.x < it.x &&
							region1.x + region1.width > it.x && it.uuid != region1.uuid) {
							var fix = 0
							if(region1.height % 2 == 1) fix = 1
							region1.height = (region1.height - 10) / 2
							it.width = region1.width
		...
		for (it : element.rootElement.rootStates.head.regions) {
			...
			if (it.uuid == element.uuid) {
				...
				if (element.rootElement.rootStates.head.regions.size == 1) {
					...
					for (region1 : element.rootElement.rootStates.head.regions) {
						if ((it.x == region1.x) && continue && region1.width == it.width &&
							(it.y - region1.y - region1.height < 13 && it.y - region1.y - region1.height > 5) &&
							it.uuid != region1.uuid) {
							region1.height = region1.height + it.height + 10
							continue = false
						} else if ((it.y == region1.y) && continue && region1.height == it.height &&
							(it.x - region1.x - region1.width < 13 && it.x - region1.x - region1.width > 5) &&
							it.uuid != region1.uuid) {
					...
					if (continue) {
						for (region1 : element.rootElement.rootStates.head.regions) {
							...
							if ((it.x == region1.x) && continue && region1.width == it.width &&
								(region1.y - it.y - it.height < 13 && region1.y - it.y - it.height > 5) &&
								it.uuid != region1.uuid) {
								region1.y = it.y
								region1.height = region1.height + it.height + 10
								continue = false
							} else if ((it.y == region1.y) && continue && region1.height == it.height &&
								(region1.x - it.x - it.width < 13 && region1.x - it.x - it.width > 5) &&
								it.uuid != region1.uuid) {
								region1.x = it.x
								region1.width = region1.width + it.width + 10
								continue = false
					...
					if (continue) {
						for (region1 : element.rootElement.rootStates.head.regions) {
							// deleted region is left region1
							if ((region1.y - it.y - it.height < 13 && region1.y - it.y - it.height > 5) &&
								it.uuid != region1.uuid) {
								region1.y = it.y
								region1.height = region1.height + it.height + 10
								continue = false
					...
					if (continue) {
						for (region1 : element.rootElement.rootStates.head.regions) {
							// deleted region is left region1
							if ((region1.x - it.x - it.width < 13 && region1.x - it.x - it.width > 5) &&
								it.uuid != region1.uuid) {
								region1.x = it.x
								region1.width = region1.width + it.width + 10
								continue = false
					...
					if (continue) {
						for (region1 : element.rootElement.rootStates.head.regions) {
							// deleted region is left region1
							if ((it.y - region1.y - region1.height < 13 && it.y - region1.y - region1.height > 5) &&
								it.uuid != region1.uuid) {
								region1.height = region1.height + it.height + 10
								continue = false
							}
					...
					if (continue) {
						for (region1 : element.rootElement.rootStates.head.regions) {
							// deleted region is left region1
							if ((it.x - region1.x - region1.width < 13 && it.x - region1.x - region1.width > 5) &&
								it.uuid != region1.uuid) {
								region1.width = region1.width + it.width + 10
								continue = false
							}
